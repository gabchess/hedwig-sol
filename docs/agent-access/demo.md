# Rebalance authorization rehearsal

Status: proposed Replit experience for a generic vault operator or rebalance
bot. Vault positions and a proposed rebalance are simulated. The only onchain
action is the existing protected counter change. This demo transfers no assets,
...
operation. The Replit screen shows simulated vault positions and a proposed
...
## One task, one role, one denied retry
...
4. The admin revokes membership and the UI waits for chain confirmation.
...
Show the actor, role, expiry, confirmed transaction status, and counter before
and after each attempt. A UI state label alone is not proof. The denied retry
...
path, not a customer integration or a finished Replit app. See the
